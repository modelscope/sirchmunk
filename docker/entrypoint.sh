...
for var in LLM_BASE_URL LLM_API_KEY LLM_MODEL_NAME LLM_TIMEOUT \
           ...
           CHAT_HISTORY_MAX_TURNS CHAT_HISTORY_MAX_TOKENS \
           SIRCHMUNK_UPLOAD_ENABLED SIRCHMUNK_UPLOAD_MAX_FILE_SIZE SIRCHMUNK_UPLOAD_MAX_TOTAL \
           SIRCHMUNK_API_TOKEN SIRCHMUNK_ALLOWED_ORIGINS SIRCHMUNK_ALLOWED_PATHS \
           SIRCHMUNK_DEBUG SIRCHMUNK_MAX_WS_CONNECTIONS; do
    val="${!var}"
    if [ -n "$val" ]; then
        python3 -c "
import sys, pathlib
