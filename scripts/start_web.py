    ...
    try:
        if os.name == "nt":
            ...
            try:
                ...
                print_flush(
                    f"   ⚠️ {name} did not terminate in {timeout}s, sending SIGKILL..."
                )
...
def start_backend():
    ...
    # Get port from environment variable (default: 8584)
    ...
    # Use uvicorn to run the FastAPI application directly
    cmd = [
        sys.executable,
        "-m",
        "uvicorn",
...
if __name__ == "__main__":
    ...
    print_flush("=" * 50)
    print_flush("Sirchmunk Web UI Launcher")
    print_flush("=" * 50)
